...
int main(int argc, char** argv) {
	...
	if(myRank==0) //Master bilgisayar icin. 
	{
		...
		/*Master tarafindan kismi boyut, kismi satir, kismi sutun 
		kumedeki diger bilgisayarlara gonderilme islemi yapilacak. */
		...
		for(int i=1; i<size; i++)
			MPI_Send(&kismiSutun, 1, MPI_INT, i, 25, MPI_COMM_WORLD);
		...
		for(int i=1; i<size; i++)
			MPI_Send(&girisDizisiPtr[i*kismiBoyut],kismiBoyut,MPI_INT,i,25,MPI_COMM_WORLD); 
		startTime=MPI_Wtime();	
		for(int i=0; i<kismiSatir; i++)
		{
			for(int j=0; j<kismiSutun; j++) 
			{
				...
				//sadece filtreleme için süre ölçeceðiz
			
				else 
				{ 

						siralananDizi[0] = girisDizisiPtr[(i-1)*kismiSutun+(j-1)];
						siralananDizi[1] = girisDizisiPtr[(i-1)*kismiSutun+(j)];
						siralananDizi[2] = girisDizisiPtr[(i-1)*kismiSutun+(j+1)];
						siralananDizi[3] = girisDizisiPtr[(i)*kismiSutun+(j-1)];
						siralananDizi[4] = girisDizisiPtr[(i)*kismiSutun+(j)];
						siralananDizi[5] = girisDizisiPtr[(i)*kismiSutun+(j+1)];
						siralananDizi[6] = girisDizisiPtr[(i+1)*kismiSutun+(j-1)];
						siralananDizi[7] = girisDizisiPtr[(i+1)*kismiSutun+(j)];
						siralananDizi[8] = girisDizisiPtr[(i+1)*kismiSutun+(j+1)];
					...
					sonucMatrisiPtr[diziIJ] = medyan;
					
				}
			}
		}
		
		for(int i=1; i<size; i++) //Filtrelenmiþ kismiSonucMatrisi ni recive ediyoruz.
			MPI_Recv(&sonucMatrisiPtr[i*kismiBoyut],kismiBoyut, MPI_INT,i,25, MPI_COMM_WORLD,&status);
	...
	else //Diger bilgisayarlar icin.
	{
		MPI_Recv(&kismiBoyut,1,MPI_INT,0,25,MPI_COMM_WORLD,&status);
		MPI_Recv(&kismiSatir,1,MPI_INT,0,25,MPI_COMM_WORLD,&status);
		MPI_Recv(&kismiSutun,1,MPI_INT,0,25,MPI_COMM_WORLD,&status);
		//Alan rezerve ettik.
		kismiGirisDizisiPtr = new int[kismiBoyut];
		kismiSonucMatrisiPtr = new int[kismiBoyut];	
		MPI_Recv(kismiGirisDizisiPtr,kismiBoyut,MPI_INT,0,25,MPI_COMM_WORLD,&status);
		...
		MPI_Send(kismiSonucMatrisiPtr,kismiBoyut,MPI_INT,0,25,MPI_COMM_WORLD);
